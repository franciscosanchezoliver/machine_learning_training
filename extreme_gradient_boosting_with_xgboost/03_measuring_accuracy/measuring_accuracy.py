...
Measuring accuracy
------------------
You'll now practice using XGBoost's learning API through its baked in cross-validation capabilities.
XGBoost gets its performance and efficiency gains by utilizing its own optimized data
structure for datasets called a DMatrix.

In the previous exercise, the input datasets were converted into DMatrix data on the fly,
but when you use the xgboost cv object, you have to first explicitly convert your
data into a DMatrix. So, that's what you will do here before running cross-validation on churn_data.
...
- Perform 3-fold cross-validation by calling xgb.cv().
    ...
    - metrics is the metric you want to compute (this will be "error", which we will convert to
      an accuracy).
...
# Read the data
churn_data = pd.read_csv('../01_xgboost_fit_predict/churn_data.csv')
...
# To tell an accuracy we can get the last test se accuracy
